...
def get_model(point_cloud, num_part, num_ins, is_training, weight_decay=0.0, bn_decay=None):
    ...
    seg_net = l0_points
    seg_net = tf_util.conv1d(seg_net, 256, 1, padding='VALID', bn=True, is_training=is_training, scope='seg/fc1',
                             bn_decay=bn_decay)
    seg_net = tf_util.conv1d(seg_net, 256, 1, padding='VALID', bn=True, is_training=is_training, scope='seg/fc2',
                             bn_decay=bn_decay)
    seg_net = tf_util.conv1d(seg_net, 128, 1, padding='VALID', bn=True, is_training=is_training, scope='seg/fc3',
                             bn_decay=bn_decay)
    # also predict the semantic part "other"
    seg_net = tf_util.conv1d(seg_net, num_part + 1, 1, padding='VALID', bn=False, activation_fn=None, scope='seg/fc4',
                             bn_decay=bn_decay)

    # instance segmentation branch
    ins_net = l0_points
    ins_net = tf_util.conv1d(ins_net, 256, 1, padding='VALID', bn=True, is_training=is_training, scope='ins/fc1',
                             bn_decay=bn_decay)
    ins_net = tf_util.conv1d(ins_net, 256, 1, padding='VALID', bn=True, is_training=is_training, scope='ins/fc2',
                             bn_decay=bn_decay)
    ins_net = tf_util.conv1d(ins_net, 128, 1, padding='VALID', bn=True, is_training=is_training, scope='ins/fc3',
                             bn_decay=bn_decay)
    # also predict the semantic part "other"
    ins_net = tf_util.conv1d(ins_net, num_ins + 1, 1, padding='VALID', bn=False, activation_fn=None, scope='ins/fc4',
                             bn_decay=bn_decay)
    ...
    conf_net = tf_util.fully_connected(conf_net, 256, bn=True, is_training=is_training, scope='conf/fc1',
                                       bn_decay=bn_decay)
    conf_net = tf_util.fully_connected(conf_net, 256, bn=True, is_training=is_training, scope='conf/fc2',
                                       bn_decay=bn_decay)
...
def iou(pred_x, gt_x, gt_conf, n_point, nmask, end_points):
    ...
    matching_idx_row = tf.concat(
        (tf.expand_dims(tf.cast(idx[:, 0], tf.int32), -1), tf.reshape(matching_idx_row, [-1, 1])), 1)
    ...
    matching_idx_column = tf.concat(
        (tf.expand_dims(tf.cast(idx[:, 0], tf.int32), -1), tf.reshape(matching_idx_column, [-1, 1])), 1)
    ...
    iou_all = tf.divide(matching_score,
                        tf.reduce_sum(gt_x_matched, 2) + tf.reduce_sum(pred_x_matched, 2) - matching_score + 1e-8)
...
def get_conf_loss(conf_pred, gt_valid, end_points):
    ...
    pos_per_shape_loss = tf.divide(tf.reduce_sum(target_pos_mask * per_part_loss, axis=-1),
                                   tf.maximum(1e-6, tf.reduce_sum(target_pos_mask, axis=-1)))
    neg_per_shape_loss = tf.divide(tf.reduce_sum(target_neg_mask * per_part_loss, axis=-1),
                                   tf.maximum(1e-6, tf.reduce_sum(target_neg_mask, axis=-1)))
...
def get_other_ins_loss(other_mask_pred, gt_other_mask, end_points):
    ...
    iou = tf.divide(matching_score, tf.reduce_sum(other_mask_pred, axis=-1) + tf.reduce_sum(gt_other_mask,
                                                                                            axis=-1) - matching_score + 1e-8)
